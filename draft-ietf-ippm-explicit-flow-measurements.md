...
signal. This method is fully based on the Alternate-Marking method {{AltMark}} and 
the Q bit represents the "packet color" that allows to mark consecutive blocks of 
packets with different colors.

{{AltMark}} introduces two variations of the Alternate-Marking method depending on 
whether the color is switched according to a fixed timer or after a fixed number of 
packets. The method based on fixed timer is described in detail in {{AltMark}}. 
It can be applied in general to measure packet loss for any protocol layer and on 
any segment of a network. Since it is required for any measurement points to 
identify the exact block to correctly compare the counters, the time length of the 
blocks must be chosen large enough to guarantee the proper operation with respect 
to clock errors and network delay issues. Also, the time length can be chosen 
according to the desired frequency for the measures.

The Q bit method, described in this document, chooses the color-switching method 
based on a fixed number of packets for each block. This approach has the advantage 
that the endpoints do not need to report the number of marked packets per block to 
the on-path probes. In this way, each probe can measure packet loss autonomously 
without relying on an external Network Management System (NMS). In addition, the 
requirement on the synchronization between endpoints and probes does not have to 
be strict. Indeed, for the purpose of the packet loss measurement, all blocks have 
the same number of packets and it is necessary to detect only the loss event and 
not to identify the exact block with losses.
